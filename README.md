...
ID와 명칭은 미리 채워뒀다. 기계적인 옮겨 적기라 판단이 안 들어가고, 그 시간을 빼면 파이프라인 쪽이 덕을 못 본다.
...
실제로 그런 사고를 봤다. 본문이 요구하는 것과 단서가 요구하는 것이 반대였고, 본문만 읽은 해석이 그대로 산출물에 들어갔다.
...
같은 제약이 한쪽에선 미덕이고 다른 쪽에선 비용이었다. 제약을 없앨 게 아니라, 제약 때문에 잘리는 것을 다른 칸에서 받기로 했다.
...
이 저장소는 "쟀느냐"를 계속 따지는데, 정작 눈으로 확인하는 걸 건너뛴 자리가 있었다.
