...
def main():
    ...
    S.append(section(
        ...
        "<b>다수결을 적용해도 일치율은 오르지 않았다.</b> 표가 갈린 항목이 1건뿐이고, "
        "그 1건에서 다수결이 고른 답이 사람과 달랐기 때문이다. "
        "반복 실행이 주는 것은 <b>정확도가 아니라 재현성</b>이다 — 단발 실행에서 "
        "흔들림을 판단 차이로 오해하는 일을 막아준다."))
    ...
    S.append(section(
        "혼동 행렬", "Q4",
        heat(cols, rows, mx),
        "<b>오류가 한 방향으로만 난다.</b> 사람이 무엇이라 했든 기계는 <code>권한절차</code>로 몰았다 — "
        "금지 3/3, 제재 3/3이 전부 그리로 갔다. "
        "무작위 오류가 아니라 <b>판정 규칙이 절차 쪽으로 기울어 있다는 신호</b>이고, 규칙을 고쳐야 잡힌다."))
    ...
    S.append(section(
        ...
        + bars([("사람", 0, "0/4"), ("파이프라인(최대)", 1, "1/4"), ("기계 스캔", 4, "4/4")],
               4, "건", note="단서 반영 건수"),
        "판정 기준을 바꿔도 <b>27~34% 안에 있다.</b> 그런데 요약이 그걸 살려낸 비율은 사람도 기계도 바닥이다. "
    ...
    S.append(section(
        ...
        "즉 <b>확신도는 대리지표로 때울 수 없고 사람에게 직접 물어야 한다.</b> "
        "이 표는 실패한 가설의 기록이고, 그래서 남긴다."))
...
<p class="lede">공개 법령 문서에서 항목을 뽑아 구조화 표로 만드는 작업을, 사람과 파이프라인으로 나눠 잰 결과.
전부 <code>data/benchmark.db</code>에 대한 SQL 한 벌에서 나왔고 쿼리는 <code>sql/analysis.sql</code>에 있다.</p>
{{BODY}}
<footer>
표본은 10건이다. 통계적 유의성을 주장하지 않는다 — “대략 몇 배”까지가 이 표본이 말할 수 있는 전부다.<br>
골든셋은 1인 1회의 라벨이다. 정답의 표준이 아니라 <b>이 측정에서의 기준</b>이라는 뜻이다.<br>
